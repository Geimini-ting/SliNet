...
class DWConv(nn.Module):
    def __init__(self, c1, c2):
        super().__init__()
        self.pointwise = nn.Conv2d(c1, c1, kernel_size=3, padding=1, groups=c1)
        self.depthwise = nn.Conv2d(c1, c2, kernel_size=1)
...
class SpatialAttentionModule(nn.Module):
    def __init__(self):
        ...
        self.conv2d = Conv(c1=2, c2=1, k=7, s=1, p=3)
        # self.act=SiLU()
        self.sigmoid = nn.Sigmoid()
...
class CBAM(nn.Module):
    ...
    def forward(self, x):
        ...
        out_s = self.spatial_attention(out)
        out = out_s * out
        # ##########    skew           ##########
        # mean = torch.mean(out_s)
        # diffs = out_s - mean
        ...
        # print("final:", final)
        # ##########    print           ##########
        # output_file_path = '/home/haochuanyan/ZH/tph-yolov5-main/skew_CBAM.txt'
        ...
        #         out_sk = skews.item()
        #         out_k = kurtoses.item()
        #         file.write(str(out_m) + "\t" + str(out_s) + "\t" + str(out_sk) + "\t" + str(out_k) + "\n")
        return out
...
class SPP(nn.Module):
    ...
    def __init__(self, c1, c2, k=(3, 5, 9, 13)):
        super().__init__()
    ...
    def forward(self, imgs, size=640, augment=False, profile=False):
        ...
        with amp.autocast(enabled=p.device.type != 'cpu'):
            ...
            y = non_max_suppression(y, self.conf, iou_thres=self.iou, classes=self.classes,
                                    multi_label=self.multi_label,  type='NMS')  # NMS
            for i in range(n):
...
class ResidualAttentionModule(nn.Module):
    def __init__(self, c1, c2, reduction=16, dimension=1):
        super().__init__()
        self.Conv_2_1 = Conv(c1=2, c2=1, k=7, s=1, p=3)
        # self.DWConv = DWConv(c1, c2)
        mid_channel = c1 // reduction  # reduction
        self.fc = nn.Sequential(
            # 全连接层
            # nn.Linear(in_features=c1, out_features=mid_channel),
            # nn.ReLU(),
            # nn.Linear(in_features=mid_channel, out_features=c1)
            nn.Conv2d(c1, mid_channel, kernel_size=1),
            nn.LayerNorm([mid_channel, 1, 1]),
            nn.ReLU(inplace=True),  # yapf: disable
            nn.Conv2d(mid_channel, c1, kernel_size=1)
        )
        self.sigmoid = nn.Sigmoid()
        self.maxchannel_pool = nn.AdaptiveMaxPool2d(1)
        self.avgchannel_pool = nn.AdaptiveAvgPool2d(1)
    ...
    def forward(self, x):
        ...
        # [N, C, 1, 1]
        avg = self.fc(self.avgchannel_pool)
        max = self.fc(self.maxchannel_pool)
        # pooling_mul_term = torch.sigmoid(avg + max)
        channel_mul_term = torch.sigmoid(self.channel_mul_conv(context))
        # feature = self.sigmoid(pooling_mul_term + channel_mul_term)
        out = x * channel_mul_term

        # ##########    skew           ##########
        # mean = torch.mean(RSA_out)
        # diffs = RSA_out - mean
        # var = torch.mean(torch.pow(diffs, 2.0))
        # std = torch.pow(var, 0.5)
        # zscores = diffs / std
        ...
        # print("final:", final)
        # ##########    print           ##########
        # output_file_path = '/home/haochuanyan/ZH/tph-yolov5-main/skew_our.txt'
        # if final is not None:
        #     with open(output_file_path, 'a') as file:
        #         out_m = mean.item()
        #         out_s = std.item()
...
class RP(nn.Module):
    def __init__(self, c1, c2, num_layers):
        ...
        self.Conv_k1 = Conv(c1, c2, k=1, s=1)
        self.Conv_k3 = Conv(c1, c1, k=3, s=2)
...
class RAM(nn.Module):
    def __init__(self, c1, c2):
        ...
        self.RAM = ResidualAttentionModule(c1, c2)
